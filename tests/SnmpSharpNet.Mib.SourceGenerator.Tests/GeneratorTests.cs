...
public class SourceGeneratorIntegrationTests
{
    private static readonly string GeneratedDir = typeof(SourceGeneratorIntegrationTests).Assembly
        ...
        .Single(attribute => attribute.Key == "CompilerGeneratedFilesOutputPath")
        .Value
        ?? throw new InvalidOperationException("The compiler generated-files output path is not available.");

    private static string FindGeneratedSource(string typeDeclaration)
    {
        return Directory.GetFiles(GeneratedDir, "*.generated.cs", SearchOption.AllDirectories)
            .Select(File.ReadAllText)
            .Single(source => source.Contains(typeDeclaration, StringComparison.Ordinal));
    }
    ...
    public async Task GeneratedStandaloneLeaf_IncludesOidMarkerAndDescription()
    {
        ...
        await Assert.That(source.Contains("/// Generated by SnmpSharpNet.Mib.SourceGenerator", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// The number of times a VLAN entry has been deleted from", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// the dot1qVlanCurrentTable (for any reason).  If an entry", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// is deleted, then inserted, and then deleted, this", StringComparison.Ordinal)).IsTrue();
    ...
    public async Task GeneratedTableEntry_CommentsPreserveOrdinalsAndNormalizeDescriptions()
    {
        ...
        await Assert.That(source.Contains("/// A table containing static configuration information for", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// each VLAN configured into the device by (local or", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// network) management.  All entries are permanent and will", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// be restored after the device is reset.", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// 1.3.6.1.2.1.17.7.1.4.3.1", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// Generated by SnmpSharpNet.Mib.SourceGenerator", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// Static information for a VLAN configured into the", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// device by (local or network) management.", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("public class Dot1qVlanStaticTableEntry : ISnmpTableEntry<Dot1qVlanStaticTableEntry>", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// Index #1", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// The VLAN-ID or other identifier referring to this VLAN.", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("public required uint IndexDot1qVlanIndex;", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// Column #1", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// An administratively assigned string, which may be used", StringComparison.Ordinal)).IsTrue();
    ...
    public async Task GeneratedNestedLeafClasses_IncludeEscapedMultilineDescriptions()
    {
        ...
        await Assert.That(source.Contains("public class CustomIndex : ISnmpLeaf<Integer32>", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("/// Column line 1", StringComparison.Ordinal)).IsTrue();
        await Assert.That(source.Contains("public class CustomValue : ISnmpLeaf<Integer32>", StringComparison.Ordinal)).IsTrue();
    }
