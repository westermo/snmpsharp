	...
	public class UserSecurityModel : AsnType, ICloneable
	{
		...
		public override void encode(MutableByte buffer)
		{
			...
			if (_authentication != AuthenticationDigests.None)
			{
				...
				{
					if (_authentication == AuthenticationDigests.SHA256)
					{
						// If authentication is used, set authentication parameters field to 24 bytes set to 0x00 for sha256
						_authenticationParameters.Set(new byte[] { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 });
					}
					else
					{
						// If authentication is used, set authentication parameters field to 12 bytes set to 0x00 for md5 or sha1
						_authenticationParameters.Set(new byte[]
