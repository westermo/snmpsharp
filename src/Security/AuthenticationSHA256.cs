...
namespace SnmpSharpNet
{
	...
	/// </summary>
	public class AuthenticationSHA256 : IAuthenticationDigest
	{
		/// <summary>
		/// Standard constructor.
		/// </summary>
		public AuthenticationSHA256()
		{
		}
		/// <summary>
		...
		public byte[] authenticate(byte[] authenticationSecret, byte[] engineId, byte[] wholeMessage)
		{
			...
			// copy 24 bytes of the hash into the wholeMessage
			for (int i = 0; i < 24; i++)
			{
				result[i] = hash[i];
			}
		...
		/// <summary>
		/// Verifies correct SHA-256 authentication of the frame. Prior to calling this method, you have to extract authentication
		/// parameters from the wholeMessage and reset authenticationParameters field in the USM information block to 24 0x00
		/// values.
		/// </summary>
		...
		public bool authenticateIncomingMsg(byte[] userPassword, byte[] engineId, byte[] authenticationParameters, MutableByte wholeMessage)
		{
			...
			HMACSHA256 sha = new HMACSHA256(authKey);
			byte[] hash = sha.ComputeHash(wholeMessage);
			MutableByte myhash = new MutableByte(hash, 24);
			sha.Clear(); // release resources
		...
		/// <summary>
		/// Verify SHA-256 authentication of a packet.
		/// </summary>
		/// <param name="authKey">Authentication key (not password)</param>
